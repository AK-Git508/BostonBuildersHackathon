...
const MAX_DRUGS = 10;
const DRUG_COLORS = [
    '#00d4ff', '#ff6b9d', '#ffd60a', '#7c3aed',
    '#10b981', '#f97316', '#ef4444', '#06b6d4',
    '#84cc16', '#ec4899'
];
const DRUG_COLORS_BG = [
    'rgba(0,212,255,0.12)',   'rgba(255,107,157,0.12)', 'rgba(255,214,10,0.12)',  'rgba(124,58,237,0.12)',
    'rgba(16,185,129,0.12)', 'rgba(249,115,22,0.12)',  'rgba(239,68,68,0.12)',   'rgba(6,182,212,0.12)',
    'rgba(132,204,22,0.12)', 'rgba(236,72,153,0.12)'
];
...
const ORGAN_ROLES = {
    brain:   { name: 'Brain / CNS',   role: 'Target & Distribution', desc: 'CNS penetration depends on lipophilicity and P-glycoprotein efflux. High drug levels here affect mood, cognition, and autonomic control.' },
    heart:   { name: 'Heart',         role: 'Circulation',           desc: 'Pumps drug through systemic circulation. Cardiac output affects distribution speed; some drugs directly alter heart rate and rhythm.' },
    lungs:   { name: 'Lungs',         role: 'Pulmonary circulation',  desc: 'Receives entire cardiac output each cycle. Drugs that alter pulmonary vasculature can affect oxygenation and gas exchange.' },
    liver:   { name: 'Liver',         role: 'Metabolism (CYP450)',    desc: 'Primary site of drug metabolism via CYP enzymes (CYP3A4, 2D6, 2C9). First-pass effect reduces oral bioavailability. Drug interactions here are most common.' },
    gi:      { name: 'GI Tract',      role: 'Absorption',            desc: 'Primary oral absorption site. P-gp efflux and gut-wall CYP3A4 affect bioavailability before systemic entry. Food timing matters here.' },
    kidneys: { name: 'Kidneys',       role: 'Excretion (renal)',      desc: 'Primary elimination route for hydrophilic drugs. Reduced eGFR slows clearance, causing drug accumulation and toxicity risk.' },
    blood:   { name: 'Blood / Plasma',role: 'Distribution vehicle',  desc: 'Drug transported bound to plasma proteins (albumin). Displacement interactions raise free drug concentration and amplify effects.' },
};
...
function getInteractionsWithIds() {
    ...
    for (let i = 0; i < activeSlots.length; i++) {
        for (let j = i + 1; j < activeSlots.length; j++) {
            ...
            const ix = drugA.interactions?.[activeSlots[j].drugId] || drugB.interactions?.[activeSlots[i].drugId];
            if (ix) found.push({
                drugA: drugA.name, drugB: drugB.name,
                drugAId: activeSlots[i].drugId, drugBId: activeSlots[j].drugId,
                ...ix
            });
        }
    }
...
function switchAISubtab(subtab) {
    ...
    document.getElementById('discoverySubtab').style.display = subtab === 'discovery' ? 'flex' : 'none';
    document.getElementById('chatbotSubtab').style.display = subtab === 'chatbot' ? 'flex' : 'none';
...
function addDrugSlot() {
    if (state.slots.length >= MAX_DRUGS) { showToast(`Maximum ${MAX_DRUGS} drugs at once`, 'warning'); return; }
...
function updateBodyAtTime(t) {
    ...
    activeSlots.forEach((slot, localIdx) => {
        ...
        drug.affectedOrgans.forEach(organKey => {
            (ORGAN_SVG_MAP[organKey] || []).forEach(svgId => {
                if (!organData[svgId]) organData[svgId] = { organKey, drugs: [], maxIntensity: 0 };
                organData[svgId].drugs.push({ drugId: slot.drugId, localIdx, intensity, concentration });
                organData[svgId].maxIntensity = Math.max(organData[svgId].maxIntensity, intensity);
            });
        });
    });

    // Determine ADME phase label for current time
    const avgTmax = results.length ? results.reduce((s, r) => s + r.tmax, 0) / results.length : 6;
    ...
    const phaseColors = { Absorption: '#00d4ff', Distribution: '#9d4edd', Metabolism: '#ff6b35', Excretion: '#ff3366' };
    if (phaseEl) {
    ...
    Object.entries(organData).forEach(([svgId, data]) => {
        ...
        const dangerIx = highIxIds.find(ix =>
            drugIds.includes(ix.drugAId) && drugIds.includes(ix.drugBId)
        );
        const isDanger = !!dangerIx && maxIntensity > 0.15;

        state.organSnapshot[svgId] = { organKey, drugs, maxIntensity, isDanger, phase, dangerMechanism: dangerIx?.mechanism || '' };
        ...
        if (isDanger) {
            ...
            el.style.strokeWidth = `${1 + maxIntensity * 1.5}`;
            el.style.filter  = maxIntensity > 0.1 ? `url(#organGlow${drugs[0].localIdx % MAX_DRUGS})` : '';
...
function showOrganTooltip(svgId, organKey, event) {
    ...
    if (!snap || snap.maxIntensity < 0.01) {
        ...
        const drugLines = snap.drugs.map(d => {
...
function showOrganPanel(svgId, organKey) {
    ...
    if (!panel) return;
    const role = ORGAN_ROLES[organKey] || { name: organKey, role: '—', desc: 'No data available.' };
    const snap = state.organSnapshot[svgId];
    ...
    if (snap && snap.maxIntensity > 0.01) {
        const drugRows = snap.drugs.map(d => {
            const drug = getDrugData(d.drugId);
            const pct = Math.round(d.intensity * 100);
...
function renderOrganLegend(activeSlots, organDrugMap) {
    ...
    if (!Object.keys(displayMap).length) {
        legend.innerHTML = colorKey + '<p class="placeholder-text" style="margin-top:0.75rem;">Run simulation to see distribution.</p>';
        return;
    }

    const organItems = Object.entries(displayMap).map(([organ, indexSet]) => {
...
function renderChatContext() {
    ...
    if (!activeSlots.length) {
        container.innerHTML = '<p class="placeholder-text" style="color:var(--text-muted);">No simulation loaded — you can still ask any pharmacology question below.</p>';
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (badge && data.source) {
                ...
                if (src.includes('claude')) badge.textContent = '🤖 Claude AI + FDA/RxNorm';
        ...
        state.chatHistory.push({ role: 'assistant', content: reply });
        const badge = document.getElementById('chatModeBadge');
        if (badge) { badge.textContent = '📚 Local Pharmacology DB'; badge.className = 'chat-mode-badge general'; }
    }
}
...
function localPharmacologyEngine(message) {
    ...
    if (mentionedDrugs.length > 0) {
        ...
        mentionedDrugs.forEach(({ drug }) => {
            ...
            section.push(`Default dose: ${drug.defaultDose}mg | Range: ${drug.doseRange?.[0]}–${drug.doseRange?.[1]}mg`);
            section.push(`Half-life: ~${drug.halfLife}h | Primary organ: ${drug.primaryOrgan}`);
            if (drug.sideEffects?.length) section.push(`Common adverse effects: ${drug.sideEffects.join(', ')}`);
            if (drug.monitoring?.length) section.push(`Monitor: ${drug.monitoring.join(', ')}`);
            const renalNote = state.patient.kidneyFunction < 60 && drug.affectedOrgans?.includes('kidneys')
        ...
        if (mentionedDrugs.length >= 2) {
            ...
            for (let i = 0; i < mentionedDrugs.length; i++) {
                for (let j = i+1; j < mentionedDrugs.length; j++) {
                    const a = mentionedDrugs[i].drug, b = mentionedDrugs[j].drug;
                    const ix = a.interactions?.[mentionedDrugs[j].id] || b.interactions?.[mentionedDrugs[i].id];
                    if (ix) ixList.push(`⚠ **${a.name} + ${b.name}** (${ix.severity?.toUpperCase()}): ${ix.mechanism}`);
                }
            }
            if (ixList.length) parts.push('\n**Known Interactions:**\n' + ixList.join('\n'));
        }
    ...
    if (activeSlots.length > 0) {
        const drugNames = activeSlots.map(s => getDrugData(s.drugId)?.name).filter(Boolean).join(' + ');

        if (m.match(/interact|combination|together|safe.*together|risk|danger/)) {
            const ixs = getInteractions();
            if (ixs.length) return `**Interactions for ${drugNames}:**\n\n` +
                ixs.map(ix => `${ix.severity === 'high' ? '🔴' : ix.severity === 'medium' ? '🟡' : '🔵'} **${ix.drugA} + ${ix.drugB}** (${ix.severity.toUpperCase()})\n${ix.mechanism}`).join('\n\n') +
        ...
        if (m.match(/mechanism|how.*work|action|what.*do/)) {
            return activeSlots.map(s => { const d = getDrugData(s.drugId); return `**${d.name}** (${d.drugClass}):\n${d.mechanism}`; }).join('\n\n');
        }

        if (m.match(/dose|dosing|how much|mg/)) {
            const elderly = state.patient.age > 65 ? ' ⚠ Elderly patient — start at lower end of range.' : '';
            const ckd = state.patient.kidneyFunction < 60 ? ` ⚠ eGFR ${state.patient.kidneyFunction}% — check renal dosing.` : '';
        ...
        if (m.match(/half.?life|eliminat|clearance/)) {
            return activeSlots.map(s => { const d = getDrugData(s.drugId); return `**${d.name}**: t½ ~${d.halfLife}h → steady state in ~${Math.round(d.halfLife * 5)}h, dose every ~${Math.round(d.halfLife * 1.5)}h`; }).join('\n\n');
        }

        if (m.match(/side effect|adverse|toxicity|toxic|harm/)) {
            return activeSlots.map(s => { const d = getDrugData(s.drugId); return `**${d.name}**: ${d.sideEffects?.join(', ') || 'see prescribing information'}`; }).join('\n\n');
        }

        if (m.match(/monitor|watch|check|lab|test/)) {
            return activeSlots.map(s => { const d = getDrugData(s.drugId); return `**${d.name}**: Monitor — ${d.monitoring?.join(', ') || 'standard clinical parameters'}`; }).join('\n\n');
        }

        if (m.match(/organ|liver|kidney|brain|heart|lung/)) {
            return activeSlots.map(s => { const d = getDrugData(s.drugId); return `**${d.name}**: Primary organ — ${d.primaryOrgan}. Also affects: ${d.affectedOrgans?.join(', ')}.`; }).join('\n\n');
        }

        // Generic summary with simulation data
        const ixs = getInteractions();
        return `**Simulation Summary for ${drugNames}**\n` +
            `Patient: ${state.patient.age}y, ${state.patient.weight}kg, eGFR ${state.patient.kidneyFunction}%, Liver: ${state.patient.liverFunction}\n` +
            `Risk Score: **${state.riskScore ?? 'N/A'}/100**\n\n` +
            activeSlots.map(s => { const d = getDrugData(s.drugId); return `**${d.name}** — ${d.mechanism.substring(0, 100)}...`; }).join('\n') +
    ...
    const conditionCombos = [
        {
            patterns: /type\s*2\s*diabet|t2dm|blood sugar|hyperglycemi|insulin resist/,
            title: 'Type 2 Diabetes',
            combos: [
                { drugs: 'Metformin + Empagliflozin (SGLT2i)', score: 97, why: 'First-line biguanide + SGLT2 inhibitor. Empagliflozin adds cardiovascular and renal protection beyond glucose lowering. Proven mortality benefit in EMPA-REG trial.' },
                { drugs: 'Metformin + Sitagliptin (DPP-4i)', score: 89, why: 'Well tolerated, low hypoglycaemia risk, renal-dose-adjust sitagliptin below eGFR 45. Good option for elderly or frail patients.' },
                { drugs: 'Metformin + Liraglutide (GLP-1 RA)', score: 93, why: 'GLP-1 agonist provides weight loss, HbA1c reduction, and cardiovascular benefit (LEADER trial). Excellent for obese T2DM patients.' },
                { drugs: 'Glipizide + Metformin', score: 78, why: 'Sulfonylurea adds insulin secretagogue effect. Lower cost but hypoglycaemia risk; monitor blood glucose closely.' },
            ],
            monitor: 'HbA1c every 3 months, eGFR, urine albumin/creatinine ratio, blood pressure',
        },
        {
            patterns: /hypertens|high blood press|elevated bp|systolic|diastolic/,
            title: 'Hypertension',
            combos: [
                { drugs: 'Lisinopril (ACEi) + Amlodipine (CCB)', score: 96, why: 'ACCOMPLISH trial gold standard. ACEi reduces RAAS activation; CCB causes vasodilation. Complementary mechanisms with additive BP reduction.' },
                { drugs: 'Losartan (ARB) + Hydrochlorothiazide', score: 91, why: 'Fixed-dose combination available. ARB preferred over ACEi in patients with cough or angioedema. Thiazide enhances natriuresis.' },
                { drugs: 'Amlodipine + Atenolol', score: 83, why: 'CCB + beta-blocker useful in angina + hypertension. Avoid in asthma; monitor heart rate for bradycardia.' },
                { drugs: 'Lisinopril + Hydrochlorothiazide + Amlodipine', score: 94, why: 'Triple therapy for resistant hypertension. Each agent targets a different BP mechanism: RAAS, volume, and vasodilation.' },
            ],
            monitor: 'BP at each visit, serum potassium & creatinine (ACEi/ARB), heart rate (beta-blocker), ankle oedema (CCB)',
        },
        {
            patterns: /heart fail|cardiac fail|chf|reduced ejection|hfref|systolic heart/,
            title: 'Heart Failure with Reduced Ejection Fraction (HFrEF)',
            combos: [
                { drugs: 'Lisinopril + Carvedilol + Spironolactone', score: 98, why: 'Evidence-based triple therapy. ACEi reduces afterload; carvedilol reduces mortality (MERIT-HF); spironolactone reduces mortality (RALES trial). Survival benefit established.' },
                { drugs: 'Sacubitril/Valsartan + Carvedilol + Empagliflozin', score: 97, why: 'PARADIGM-HF: ARNI superior to ACEi in HFrEF. SGLT2i (EMPEROR-Reduced) adds further mortality reduction. Now guideline-directed quadruple therapy.' },
                { drugs: 'Furosemide + Spironolactone', score: 85, why: 'Congestion management: loop diuretic for symptom relief, aldosterone antagonist for mortality benefit. Monitor electrolytes closely — risk of hyperkalaemia.' },
            ],
            monitor: 'Daily weight, BP, HR, serum K+, creatinine, BNP/NT-proBNP, echo EF every 3–6 months',
        },
        {
            patterns: /depress|major depress|mdd|low mood|antidepress|ssri/,
            title: 'Major Depressive Disorder',
            combos: [
                { drugs: 'Sertraline + Bupropion', score: 88, why: 'Augmentation strategy: SSRI + NDRI covers serotonin and dopamine/norepinephrine pathways. Bupropion offsets SSRI-related sexual dysfunction and weight gain.' },
                { drugs: 'Escitalopram (SSRI) monotherapy', score: 84, why: 'Most selective SSRI, fewest drug interactions (minimal CYP2D6), well tolerated. First-line per APA guidelines. 6–8 weeks for full effect.' },
                { drugs: 'Venlafaxine (SNRI) + Mirtazapine', score: 87, why: '"California Rocket Fuel" — SNRI + NaSSA. Mirtazapine blocks α2 autoreceptors enhancing serotonin/NE release; improves sleep and appetite. Used in treatment-resistant depression.' },
                { drugs: 'Sertraline + Aripiprazole (augmentation)', score: 82, why: 'Atypical antipsychotic augmentation for partial SSRI responders. Aripiprazole at low dose (2–10mg) improves response rates in MDD.' },
            ],
            monitor: 'PHQ-9 score at each visit, suicidality assessment, weight, metabolic panel (atypical antipsychotics), QTc (if applicable)',
        },
        {
            patterns: /anxiety|gad|panic|generalised anxiety|anxious/,
            title: 'Generalized Anxiety Disorder',
            combos: [
                { drugs: 'Sertraline (SSRI) + Buspirone', score: 86, why: 'SSRI as backbone; buspirone (5-HT1A partial agonist) augments anxiolytic effect without dependence risk. Takes 2–4 weeks for buspirone to work.' },
                { drugs: 'Escitalopram + CBT (+ short-term lorazepam bridge)', score: 90, why: 'SSRI + psychotherapy is most effective long-term. Short benzodiazepine bridge for severe acute anxiety while SSRI reaches therapeutic levels (2–4 weeks).' },
                { drugs: 'Duloxetine (SNRI) + Pregabalin', score: 84, why: 'SNRI addresses anxiety and comorbid pain/depression; pregabalin (α2δ ligand) has direct anxiolytic effect. Effective for GAD with somatic symptoms.' },
            ],
            monitor: 'GAD-7 score, dependence risk if benzodiazepines used, sleep quality, BP (SNRI)',
        },
        {
            patterns: /cholesterol|dyslipid|high ldl|statin|cardiovascular risk|atheroscler/,
            title: 'Dyslipidaemia / Cardiovascular Risk Reduction',
            combos: [
                { drugs: 'Atorvastatin + Ezetimibe', score: 95, why: 'Statin inhibits cholesterol synthesis (HMG-CoA reductase); ezetimibe blocks intestinal absorption. IMPROVE-IT trial: combination reduces cardiovascular events beyond statin alone.' },
                { drugs: 'Rosuvastatin + Ezetimibe + Aspirin', score: 93, why: 'High-intensity statin + cholesterol absorption inhibitor + antiplatelet. Standard secondary prevention regimen post-MI or stroke.' },
                { drugs: 'Atorvastatin + Fenofibrate', score: 80, why: 'Statin + fibrate for mixed dyslipidaemia (high LDL + high triglycerides). Monitor for myopathy — avoid gemfibrozil with statins (CYP interaction).' },
            ],
            monitor: 'Fasting lipid panel at 4–12 weeks, LFTs at baseline, CK if myalgia symptoms, HbA1c (statins modestly raise DM risk)',
        },
        {
            patterns: /atrial fibr|afib|af\b|irregular heartbeat|rate control|rhythm/,
            title: 'Atrial Fibrillation',
            combos: [
                { drugs: 'Apixaban (DOAC) + Metoprolol (rate control)', score: 96, why: 'DOAC for stroke prevention (superior to warfarin in ARISTOTLE trial); beta-blocker for rate control. Most common AF management strategy.' },
                { drugs: 'Warfarin + Digoxin + Amiodarone', score: 72, why: 'Older regimen: warfarin anticoagulation, digoxin for rate control in HF, amiodarone for rhythm. High interaction risk — amiodarone raises warfarin INR significantly. Monitor closely.' },
                { drugs: 'Rivaroxaban + Bisoprolol', score: 94, why: 'Once-daily DOAC (convenient) + cardioselective beta-blocker for rate control. Good option for most AF patients without severe renal impairment.' },
            ],
            monitor: 'HR at rest <80 bpm, INR 2-3 (if warfarin), renal function for DOAC dosing, bleeding symptoms',
        },
        {
            patterns: /pain|analges|chronic pain|neuropath|fibromyal|musculoskel/,
            title: 'Chronic Pain / Neuropathic Pain',
            combos: [
                { drugs: 'Gabapentin + Duloxetine', score: 89, why: 'α2δ ligand reduces nerve excitability; SNRI modulates descending pain pathways. Combination covers peripheral and central sensitisation. Effective for diabetic neuropathy and fibromyalgia.' },
                { drugs: 'Acetaminophen + Ibuprofen (alternating)', score: 85, why: 'Different mechanisms: acetaminophen (central COX inhibition) + NSAID (peripheral COX-1/2). Alternating reduces individual drug load. Limit ibuprofen in renal impairment.' },
                { drugs: 'Pregabalin + Amitriptyline (low dose)', score: 82, why: 'Pregabalin for neuropathic component; low-dose TCA (10–25mg) improves sleep and amplifies pain modulation. Monitor for anticholinergic side effects in elderly.' },
            ],
            monitor: 'Pain scores (NRS), sedation/falls risk (gabapentinoids), renal function (NSAIDs), QTc (TCA)',
        },
        {
            patterns: /infect|antibiotic|pneumon|uti|sepsis|bacterial/,
            title: 'Bacterial Infections',
            combos: [
                { drugs: 'Amoxicillin + Clavulanate (Augmentin)', score: 92, why: 'Beta-lactam + beta-lactamase inhibitor. Clavulanate protects amoxicillin from resistance enzymes. Broad coverage for respiratory, skin, and urinary infections.' },
                { drugs: 'Ciprofloxacin + Metronidazole', score: 86, why: 'Fluoroquinolone (gram-negative coverage) + nitroimidazole (anaerobe coverage). Used for intra-abdominal infections and complicated UTI. Avoid in pregnancy.' },
                { drugs: 'Ceftriaxone + Azithromycin', score: 91, why: 'Standard community-acquired pneumonia combination. Cephalosporin covers gram-positive/negative; azithromycin covers atypicals (Mycoplasma, Legionella, Chlamydia).' },
            ],
            monitor: 'Culture & sensitivity before starting, signs of C. diff (diarrhoea), renal function (fluoroquinolones), QTc (azithromycin + fluoroquinolone)',
        },
        {
            patterns: /asthma|bronchospasm|wheez|reactive airway/,
            title: 'Asthma',
            combos: [
                { drugs: 'Fluticasone (ICS) + Salmeterol (LABA)', score: 95, why: 'ICS reduces airway inflammation; LABA provides sustained bronchodilation. Never use LABA without ICS in asthma. Advair/Seretide fixed-dose combination.' },
                { drugs: 'Budesonide + Formoterol (SMART therapy)', score: 97, why: 'Single inhaler for maintenance AND rescue (SMART). Formoterol\'s fast onset makes it dual-purpose. Reduces severe exacerbations versus SABA rescue alone.' },
                { drugs: 'Montelukast + Low-dose ICS', score: 82, why: 'Leukotriene receptor antagonist adds anti-inflammatory effect. Useful in allergic asthma or exercise-induced bronchoconstriction. Option when LABA not tolerated.' },
            ],
            monitor: 'Peak flow / spirometry, symptom frequency, exacerbation rate, oral candidiasis (ICS — rinse mouth after use)',
        },
        {
            patterns: /copd|emphysema|chronic obstruct|bronchitis|smoking.*lung/,
            title: 'COPD',
            combos: [
                { drugs: 'Tiotropium (LAMA) + Salmeterol (LABA)', score: 94, why: 'Dual bronchodilation: anticholinergic (reduces airway tone) + beta-2 agonist. Reduces exacerbations and improves exercise tolerance. GOLD guideline Group B/C/D.' },
                { drugs: 'Tiotropium + Fluticasone/Salmeterol (ICS/LABA)', score: 88, why: 'Triple therapy for frequent exacerbators (≥2/year or 1 requiring hospitalisation). ICS reduces eosinophilic inflammation. Monitor pneumonia risk with ICS.' },
                { drugs: 'Roflumilast + LAMA', score: 79, why: 'PDE4 inhibitor (roflumilast) reduces inflammation in chronic bronchitis phenotype. Add-on for patients with FEV1 <50% and frequent exacerbations despite bronchodilators.' },
            ],
            monitor: 'Spirometry annually, exacerbation frequency, O2 saturations, BMI, 6-minute walk test',
        },
    ];
